...
// return a pointer to the inserted node
  ...
  Node* Node::insert_before(int d) {
    Node* newnode= new Node(d, previous, this); //1 operation
    previous->next=newnode; //1 operation
    previous= newnode;  //1 operation
    
    return newnode;  //f(n)=3=O(1)
}
...
// return a pointer to the inserted node
 Node* Node::insert_after(int d) {
  ...
  /* Complete this function */
    Node* newnode= new Node(d, this, next); //1 operation
    next->previous=newnode;   //1 operation
    next= newnode;            //1 operation
...
{
  /* Complete this function */
    Node* temp= this->previous;      //1 operation
    previous->previous->next=this;   //1 operation
    previous=previous->previous;     //1 operation
...
void Node::delete_after()
{
  ...
  /* Complete this function */
    Node* temp= this->next;              //1 operaiton
    next->next->previous=this;           //1 operaiotn
    next=next->next;                     //1 operation

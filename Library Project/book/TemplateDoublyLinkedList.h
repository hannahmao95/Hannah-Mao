...
public:
    ...
    DListNode <T> * insertOrderly(const T& Obj);// insert an object to the correct position
};
...
DListNode<T>* DoublyLinkedList<T>::insertOrderly(const T& obj){
    ...
    DListNode<T>* tmp = header.getNext();  //1 operation
    //worst case O(n)
    while(tmp != this->getAfterLast()){  //n iteration
        if(obj < tmp->getElem()){
            DListNode<T>* node= new DListNode<T>(obj, tmp->getPrev(), tmp);
            tmp->prev->next= node;    // 1 operation
            tmp->prev = node;        //1 operation
            return node;
        }
        tmp = tmp-> getNext(); //1 operation
    }
    
    DListNode<T> *node = new DListNode<T>(obj, trailer. prev,&trailer);  //1 operation
    trailer.prev -> next = node;  //1 operaion
    trailer.prev = node;         //1 operation
    return  node;
}  //f(n)= 1+3n+3=3n+4=O(n)
